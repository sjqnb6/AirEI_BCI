...
import CytonElectrodeStatus_.CytonElectrodeLocations;
import CytonElectrodeStatus_.CytonElectrodeStatus;
import CytonElectrodeStatus_.ElectrodeState;
import CytonImpedanceEnums_.CytonImpedanceInterval;
...
public class W_CytonImpedance extends Widget {
    ...
    public W_CytonImpedance(GUI MAIN){
        ...
        threshold_ui_cp5 = new ControlP5(pApplet);
        threshold_ui_cp5.setGraphics(pApplet, 0,0);
        threshold_ui_cp5.setAutoDraw(false);

        checkingImpedanceOnElectrodeGif = new Gif(pApplet, "Rolling-1s-200px.gif");
    ...
    public void update(){
        ...
        for (int i = 0; i < cytonElectrodeStatus.length; i++) {
            ...
            cp5ElementsToCheck.add((Controller)cytonElectrodeStatus[i].getTestingButton());
        }
        cp5ElementsToCheck.add((Controller)cytonImpedanceMasterCheck);
        cp5ElementsToCheck.add((Controller)cytonResetAllChannels);
    ...
    public void draw(){
        ...
        */
        ...
        popStyle();
        */
        pApplet.pushMatrix(); // save the transformation matrix
        pApplet.translate(translate_facepadX, translate_facepadY);
        pApplet.scale(s); // scale the transformation matrix
        /*
        pushStyle();
        stroke(0,0,0,255);
        fill(0,255,0,100);
        ...
        popStyle();
        */
        pApplet.image(cytonHeadplotStatic, 0, 0);
        pApplet.popMatrix(); // restore the transformation matrix
    ...
    public void screenResized(){
        ...
        translate_facepadY = (facepad_h < imageContainerH) ? (int) (facepad_y + (((float) imageContainerH / 2) - (facepad_h / 2))) : facepad_y;
    ...
    private void initCytonImpedanceMap() {
        if (nchan == 8) {
            cytonHeadplotStatic = pApplet.loadImage("Cyton_8Ch_Static_Headplot_Image.png");
        } else {
            cytonHeadplotStatic = pApplet.loadImage("Cyton_16Ch_Static_Headplot_Image.png");
        }
    ...
    public void drawUserLeftRightLabels() {
        ...
        float _y = y + 20;
        pApplet.text(s, _x , _y);
        s = "User Right";
        _x = (float) (translate_facepadX + facepad_w * .8);
        pApplet.text(s, _x , _y);
        pApplet.popStyle();
    }
