...
public class HeadPlot {
    ...
    public HeadPlot(PApplet _parent, int _x, int _y, int _w, int _h, int _win_x, int _win_y) {
    ...
    public void setPositionSize(int _x, int _y, int _w, int _h, int _win_x, int _win_y) {
        ...
        _x = _x + (int)((_w)*percentMargin);
        _y = _y + (int)((_h)*percentMargin)-navHeight/2;
        _w = (int)((_w)-(2*((_w)*percentMargin)));
        _h = (int)((_h)-(2*((_h)*percentMargin)));
    ...
    public void setMaxIntensity_uV(float val_uV) {
        ...
        intense_min_uV = (float) (intense_max_uV / 200.0 * 5.0f);  //set to 200, get 5
    ...
    public void setWindowDimensions(int win_width, int win_height) {
        ...
        image_y = nose_y[2];
        headImage = pApplet.createImage((int)(total_width), (int)(total_height), ARGB);

        //initialize the image
        for (int Iy=0; Iy < headImage.height; Iy++) {
            for (int Ix = 0; Ix < headImage.width; Ix++) {
                headImage.set(Ix, Iy, CP.WHITE);
            }
        }
    ...
    private void setElectrodeLocations(int n_elec, float elec_relDiam) {
        ...
        try {
            elec_relXY = pApplet.loadTable(default_fname, "header,csv"); //try loading the default file
        }
    ...
    private Table createDefaultElectrodeLocations(String fname, float elec_relDiam) {
        ...
        try {
            pApplet.saveTable(table_elec_relXY, full_fname, "csv");
        }
        catch (NullPointerException e) {
    ...
    private void computePixelWeightingFactors_multiScale(int n_wide_full, int n_tall_full) {
        ...
        for (int Ix=0; Ix<n_wide_full; Ix++) {
            ...
            dx_frac = (float)(Ix - Ix_source*decimation)/(decimation);
            for (int Iy=0; Iy < n_tall_full; Iy++) {
                int Iy_source = Iy/decimation;
                dy_frac = (float)(Iy - Iy_source*decimation)/(decimation);
        ...
        for (int Iy=0; Iy < headImage.height; Iy++) {
            ...
            for (int Ix = 0; Ix < headImage.width; Ix++) {
                ...
                if (isPixelInsideHead(pixel_x, pixel_y)==false) {
                    ...
                    for (int Ielec=0; Ielec < n_elec; Ielec++) {
                        //compute distance
                        dist = max((int) 1.0, calcDistance(pixel_x, pixel_y, electrode_xy[Ielec][0], electrode_xy[Ielec][1]));
                        if (dist < elec_radius) withinElecInd = Ielec;

                        //compute the first part of the weighting factor
                        foo_dist = max((int) 1.0, abs(dist - elec_radius));  //remove radius of the electrode
        ...
        for (int Ix=0; Ix<n_wide; Ix++) {
            for (int Iy=0; Iy<n_tall; Iy++) {
                if (withinHead[Ix][Iy]==false) {
                    ...
                    for (int Ielec=0; Ielec<n_elec; Ielec++) {
                        weightFac[Ielec][Ix][Iy]= (float) -1.0;  //this means to ignore this weight
                    }
                } else {
    ...
    private void computeWeightFactorsGivenOneElectrode_iterative(int toPixels[][][][], int toElectrodes[][][], int Ielec, float pixelVal[][][]) {
        ...
        //begin iteration
        int iter_count = 0;
        float max_dVal = (float) (10.0*dVal_threshold);  //initilize to large value to ensure that it starts
            ...
            for (int Ix=0; Ix<n_wide; Ix+=pixel_step) {
                for (int Iy=0; Iy<n_tall; Iy+=pixel_step) {
                    ...
                    if (anyConnections) {
                        ...
                        pixelVal[Ielec][Ix][Iy] = (float) -1.0; //means that there are no connections
                    }
                }
            }
    ...
    private void whereAreThePixels(int pixelAddress[][][], boolean[][] withinHead, int[][] withinElectrode) {
        ...
        int withinElecInd=-1;
        float dist;
        float elec_radius = (float) (0.5*elec_diam);
    ...
    private float calcPixelVoltage(int pixel_Ix, int pixel_Iy, float prev_val) {
        ...
        //smooth in time
        if (smooth_fac > 0.0f) voltage = (float) (smooth_fac*prev_val + (1.0-smooth_fac)*voltage);
    ...
    private int calcPixelColor(float pixel_volt_uV) {
        ...
        for (int i=0; i < 3; i++) {
            ...
            new_rgb[i] = constrain((int) new_rgb[i], (int) 0.0, (int) 255.0);
        }

        //quantize the color to make contour-style plot?
        if (true) quantizeColor(new_rgb);

        return pApplet.color((int)(new_rgb[0]), (int)(new_rgb[1]), (int)(new_rgb[2]), 255);
    }

    private void quantizeColor(float new_rgb[]) {
        int n_colors = 12;
        int ticks_per_color = 256 / (n_colors+1);
        for (int Irgb=0; Irgb<3; Irgb++) new_rgb[Irgb] = min((int) 255.0, (float)((new_rgb[Irgb]/ticks_per_color))*ticks_per_color);
    }
    ...
    private boolean isMouseOverElectrode(int n){
        float elec_mouse_x_dist = electrode_xy[n][0] - pApplet.mouseX;
        float elec_mouse_y_dist = electrode_xy[n][1] - pApplet.mouseY;
    ...
    private boolean isDraggedElecInsideHead() {
        int dx = pApplet.mouseX - circ_x;
        int dy = pApplet.mouseY - circ_y;
    ...
    void mousePressed() {
        if (mouse_over_elec_index > -1) {
            isDragging = true;
            drag_x = pApplet.mouseX - electrode_xy[mouse_over_elec_index][0];
            drag_y = pApplet.mouseY - electrode_xy[mouse_over_elec_index][1];
        } else {
            isDragging = false;
        }
    }
    ...
    void mouseDragged() {
        if (isDragging && mouse_over_elec_index > -1 && isDraggedElecInsideHead()) {
            electrode_xy[mouse_over_elec_index][0] = pApplet.mouseX - drag_x;
            electrode_xy[mouse_over_elec_index][1] = pApplet.mouseY - drag_y;
        }
    }
    ...
    public void draw() {
        ...
        pApplet.triangle(nose_x[0], nose_y[0], nose_x[1], nose_y[1], nose_x[2], nose_y[2]);  //nose
        pApplet.ellipse(earL_x, earL_y, ear_width, ear_height); //little circle for the ear
        pApplet.ellipse(earR_x, earR_y, ear_width, ear_height); //little circle for the ear

        //draw head itself
        pApplet.fill(CP.WHITE);  //fill in a white head
        pApplet.strokeWeight(1);
        pApplet.ellipse(circ_x, circ_y, circ_diam, circ_diam); //big circle for the head
        if (drawHeadAsContours) {
            //add the contnours
            pApplet.image(headImage, image_x, image_y);
            pApplet.noFill(); //overlay a circle as an outline, but no fill
            pApplet.strokeWeight(1);
            pApplet.ellipse(circ_x, circ_y, circ_diam, circ_diam); //big circle for the head
        }
        ...
        for (int Ielec=0; Ielec < electrode_xy.length; Ielec++) {
            if (drawHeadAsContours) {
                pApplet.noFill(); //make transparent to allow color to come through from below
            } else {
                pApplet.fill(electrode_rgb[0][Ielec], electrode_rgb[1][Ielec], electrode_rgb[2][Ielec]);
            }
            ...
            pApplet.ellipse(electrode_xy[Ielec][0], electrode_xy[Ielec][1], elec_diam, elec_diam); //electrode circle
        }
        ...
        for (int i=0; i < electrode_xy.length; i++) {
            ...
            pApplet.text(i+1, electrode_xy[i][0], electrode_xy[i][1]);
        }
