...
public class InterfaceSerial {
    ...
    private final float scale_fac_accel_G_per_count = (float) (0.002 / ((float)pow(2, 4)));  //assume set to +/4G, so 2 mG per digit (datasheet). Account for 4 bits unused
    ...
    public int openSerialPort(PApplet applet, String comPort, int baud) {
        ...
        try {
            ...
            changeState(applet, STATE_COMINIT);
            return 0;
        }
    ...
    public int changeState(PApplet pApplet, int newState) {
        state = newState;
        prevState_millis = pApplet.millis();
        return 0;
    }

    int finalizeCOMINIT(PApplet pApplet) {
        changeState(pApplet, STATE_NORMAL);
        return 0;
    }
    ...
    public void updateSyncState(int sdSetting) {
        ...
        if ( (pApplet.millis() - prevState_millis > COM_INIT_MSEC) && (prevState_millis != 0) && (state == STATE_COMINIT) ) {
            state = STATE_SYNCWITHHARDWARE;
            timeOfLastCommand = pApplet.millis();
        ...
        if ( (state == STATE_SYNCWITHHARDWARE) && (currentlySyncing == false) ) {
            if (pApplet.millis() - timeOfLastCommand > 200 && readyToSend == true) {
