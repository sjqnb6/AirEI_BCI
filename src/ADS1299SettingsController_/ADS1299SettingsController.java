...
public class ADS1299SettingsController{
    ...
    private final int yesOnColor = 0xDFF2BF;
    private final int noOffColor = 0xFFD2D2;
    ...
    public ADS1299SettingsController(GUI MAIN, List<Integer> _activeChannels, int _x, int _y, int _w, int _h, int _channelBarHeight) {
        x = _x;
        y = _y;
        w = _w;
        h = _h;
        ...
        this.MAIN = MAIN;
        hwsCp5 = new ControlP5(MAIN);
        hwsCp5.setGraphics(MAIN, 0,0);
    ...
    public void draw() {
        ...
        if (isVisible) {
            ...
            MAIN.stroke(MAIN.OBJECT_BORDER_GREY);
            MAIN.fill(MAIN.GREY_100);
            MAIN.rect(x, y - columnLabelH, w, columnLabelH);
            MAIN.popStyle();
            ...
            for (int i = 0; i < channelCount; i++) {
                ...
                if (hasUnappliedChanges[i]) {
                    MAIN.pushStyle();
                    MAIN.fill(MAIN.color(57, 128, 204, 190)); //light blue from TopNav
                    //fill(color(245, 64, 64, 180)); //light red
                    MAIN.rect(x, y + chanBar_h * i, w, chanBar_h);
                    MAIN.popStyle();
                }
            }
            ...
            MAIN.pushStyle();
            MAIN.fill(MAIN.GREY_100);
            MAIN.rect(x, y + h, w + 1, commandBarH);
            if (showCustomCommandUI) {
                MAIN.rect(customCmdUI_x, y + h + commandBarH, customCmdUI_w, commandBarH); //keep above style for other command buttons
            }
            MAIN.popStyle();
    ...
    private void createHWSettingsLoadButton(String name, String text, int _x, int _y, int _w, int _h) {
        loadButton = MAIN.createButton(hwsCp5, name, text, _x, _y, _w, _h);
    ...
    private void createHWSettingsSaveButton(String name, String text, int _x, int _y, int _w, int _h) {
        saveButton = MAIN.createButton(hwsCp5, name, text, _x, _y, _w, _h);
        ...
        saveButton.onClick(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                MAIN.selectOutput("Save settings to file", "storeHardwareSettings");
            }
        });
    }

    private void createHWSettingsSendButton(String name, String text, int _x, int _y, int _w, int _h) {
        sendButton = MAIN.createButton(hwsCp5, name, text, _x, _y, _w, _h);
        sendButton.setBorderColor(MAIN.OBJECT_BORDER_GREY);
    ...
    private void createCustomCommandUI() {
        customCommandTF = hwsCp5.addTextfield("customCommand")
                ...
                .setColorValueLabel(MAIN.OPENBCI_DARKBLUE)  // text color
                .setColorForeground(MAIN.OBJECT_BORDER_GREY)  // border color when not selected
        ...
        sendCustomCmdButton = MAIN.createButton(hwsCp5, "sendCustomCommand", "Send Custom Command", 0, 0, 10, 10);
        sendCustomCmdButton.setBorderColor(MAIN.OBJECT_BORDER_GREY);
        ...
        sendCustomCmdButton.onClick(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                ...
                MAIN.println("ADSSettingsController: Response == " + res.getValue());
            }
        });
    ...
    void updateHasUnappliedSettings(int _channel) {
    ...
    public void updateChanSettingsDropdowns(int chan, boolean isActive) {
        int darkNotActive = MAIN.color(57);
        int c = isActive ? MAIN.color(255) : darkNotActive;
        ...
        c = isActive ? (boardSettings.values.bias[chan] == Bias.INCLUDE ? yesOnColor : noOffColor) : (int) darkNotActive;
