...
public class W_Prediction extends Widget {
    ...
    public void draw() {
        ...
        drawPanelShadow(panelX, panelY, panelW, panelH, panelRadius, dominantIndex);
    ...
    private void drawPanelShadow(float panelX, float panelY, float panelW, float panelH, float radius, int dominantIndex) {
        pApplet.noStroke();
        // 如果是困倦状态(index == 2)，实现呼吸灯效果
        if (dominantIndex == 2) {
            float pulse = 0.5f + 0.5f * (float) Math.sin(MAIN.frameCount * 0.05f);
            pApplet.fill(231, 76, 60, 30 + 40 * pulse);
            pApplet.rect(panelX + 8, panelY + 10, panelW, panelH, radius);
            pApplet.fill(231, 76, 60, 18 + 25 * pulse);
    ...
    private void drawHeader(float headerX, float headerY, float headerW, float headerH, int dominantIndex) {
        ...
        pApplet.textSize(11);
        pApplet.text("当前疲劳等级", headerX + 18, headerY + 14);

        int accentColor = getAccentColor(dominantIndex);
        pApplet.fill(accentColor);
        pApplet.textSize(21);
        ...
        fillColorWithAlpha(accentColor, 34);
        pApplet.rect(badgeX, badgeY, badgeW, badgeH, 17f);
        
        // 困倦状态下气泡边框加入动态高亮
        if (dominantIndex == 2) {
            float pulse = 0.5f + 0.5f * (float) Math.sin(MAIN.frameCount * 0.1f);
            strokeColorWithAlpha(accentColor, 90 + (int)(100 * pulse));
            pApplet.strokeWeight(1.5f);
        } else {
            strokeColorWithAlpha(accentColor, 90);
            pApplet.strokeWeight(1.2f);
        }
    ...
    private void drawChartFrame(float chartLeft, float chartTop, float chartRight, float chartBottom, float chartWidth, float chartHeight) {
        ...
        pApplet.text("分类", chartRight + 15, chartBottom + 19);
    ...
    private void drawBars(float chartLeft, float chartBottom, float chartWidth, float chartHeight, int dominantIndex) {
        ...
        for (int i = 0; i < numClasses; i++) {
            ...
            pApplet.fill(isDominant ? getAccentColor(i) : pApplet.color(100));
    ...
    private int getAccentColor(int index) {
        int[] colors = {
                pApplet.color(46, 204, 113),  // 绿色 - 清醒
                pApplet.color(241, 196, 15),  // 黄/橘 - 疲劳
                pApplet.color(231, 76, 60)    // 红色 - 困倦
        };
