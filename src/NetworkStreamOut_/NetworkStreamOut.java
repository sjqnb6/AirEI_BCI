...
public class NetworkStreamOut extends Thread {
    ...
    private void debugTimeSeriesDataSamplingRate() {
        ...
        if (sampleRateClock == 0) sampleRateClock = pApplet.millis();
        samplesSent = samplesSent + nPointsPerUpdate;
        if (pApplet.millis() > sampleRateClock + sampleRateClockInterval) {
            float timeDelta = (float)(pApplet.millis() - sampleRateClock) / 1000;
